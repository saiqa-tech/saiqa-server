 ...
 * 
 * Per-form question overrides (WS3):
 * - `overrides` objects are extracted from question entries before sending to CheckOps.
 * - They are stored in form.metadata.questionOverrides (a plain object keyed by questionId UUID).
 * - The enricher reads them back on every form read and re-attaches them.
 * 
...
async function createFormWithQuestionIds(formData) {
    ...
    // Step 2: Extract per-form question overrides so they can be persisted in metadata.
    // CheckOps normalises questions to UUID strings internally — overrides would be lost
    // if we just passed them through. We store them in metadata.questionOverrides instead.
    const questionOverrides = {};
    for (const q of questionsWithIds) {
        const qId = q.id || q.questionId;
        if (qId && q.overrides && Object.keys(q.overrides).length > 0) {
            questionOverrides[qId] = q.overrides;
        }
    }

    // Step 3: Create form with simple UUID array (SIMPLIFIED!) + overrides in metadata
    const formDataWithQuestionIds = {
        ...formData,
        questions: questionsWithIds.map(q => q.id),  // Simple UUID array!
        metadata: {
            ...(formData.metadata ?? {}),
            ...(Object.keys(questionOverrides).length > 0
                ? { questionOverrides }
                : {}),
        },
    };
...
async function createSubmissionWithQuestionIds(submissionData) {
    const checkops = getCheckOpsWrapper();
    const { formId, submissionData: rawData, metadata, targetUnitId, submitterUserId } = submissionData;
